...
echo $($PSQL "SELECT UNNEST(ARRAY[t1.name,t2.name]) AS name FROM games INNER JOIN teams AS t1 ON games.winner_id=t1.team_id INNER JOIN teams AS t2 ON games.opponent_id=t2.team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name")
